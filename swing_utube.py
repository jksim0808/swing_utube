...
if not universe_df.empty:
    ...
    if results:
        ...
        # 🔥 오늘 날짜 계산 (예: 2024년 5월 12일)
        today_str = datetime.now(KST).strftime('%Y년 %m월 %d일')
        
        st.markdown(f'<div class="shorts-title">AI 스윙 타점 TOP 10</div>', unsafe_allow_html=True)
        # 날짜 뱃지 렌더링
        st.markdown(f'<div class="date-badge-container"><span class="date-badge">⚡ {today_str} 기준</span></div>', unsafe_allow_html=True)
        ...
        # 대본에도 오늘 날짜를 자연스럽게 추가하여 AI 성우가 읽어주도록 세팅
        script_content = f"""
        {today_str}, AI가 분석한 오늘의 스윙 타점 탑텐입니다! 
